...
use crate::{object::{self, *}, token::Token, LoxError};
...
impl Environment {
    ...
    pub fn get(&self, name: &Token) -> Result<Object, LoxError> {
        if let Some(object) = self.values.get(name.as_string()) {
            ...
            Err(LoxError::runtime_error(&name, &format!("Undefined variable '{}'.", name.as_string())))
        }
    } 
} 
...
mod tests {
    ...
    fn can_define_variable(){
        ...
        assert_eq!(e.values.get(&"One".to_string()).unwrap(), &Object::Bool(true));
    }

    #[test]
    fn can_redefine_variable(){
        let mut e = Environment::new();
        e.define(&"Two".to_string(), Object::Bool(true));
        e.define(&"Two".to_string(), Object::Num(12.0));
        assert_eq!(e.values.get(&"Two".to_string()).unwrap(), &Object::Num(12.0));
    }
